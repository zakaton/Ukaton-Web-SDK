...
{
    const eventDispatcherAddEventListener = THREE.EventDispatcher.prototype.addEventListener;
    THREE.EventDispatcher.prototype.addEventListener = function (type, listener, options) {
        if (options) {
            if (options.once) {
...
class BaseMission extends THREE.EventDispatcher {
    constructor() {
        ...
        this.motion = {
            ...
            magnetometer: new THREE.Vector3(),
            ...
            if (this.isConnected && this.disableSensorsBeforeUnload) {
                ...
                for (const sensorType in this.SensorDataTypeStrings) {
                    ...
                    this.SensorDataTypeStrings[sensorType].forEach((sensorDataType) => {
                        sensorDataConfigurations[sensorType.toLowerCase()][sensorDataType] = 0;
                    });
                }
    ...
    _onTypeUpdate() {
        this.isInsole = this._type == this.Types.LEFT_INSOLE || this._type == this.Types.RIGHT_INSOLE;
        if (this.isInsole) {
    ...
    _parseSensorDataConfigurations(dataView, byteOffset = 0) {
        ...
        this.SensorTypeStrings.forEach((sensorTypeString, sensorType) => {
            byteOffset = this._parseSensorDataConfiguration(dataView, byteOffset, sensorType);
        });
        this.dispatchEvent({
    ...
    _parseSensorDataConfiguration(dataView, byteOffset, sensorType) {
        ...
        sensorDataTypeStrings.forEach((sensorDataTypeString, index) => {
            sensorDataConfiguration[sensorDataTypeString] = dataView.getUint16(byteOffset, true);
            byteOffset += 2;
        });

        const lowerCaseSensorTypeString = sensorTypeString.toLowerCase();
        this._sensorDataConfigurations[lowerCaseSensorTypeString] = sensorDataConfiguration;
        return byteOffset;
    }
        ...
        this.SensorTypeStrings.forEach((sensorTypeString, sensorType) => {
            ...
            if (sensorTypeString in configurations && (sensorType != this.SensorTypes.PRESSURE || this.isInsole)) {
                flattenedConfigurations = this._concatenateArrayBuffers(
                    ...
                    this._flattenSensorConfiguration(configurations[sensorTypeString], sensorType)
                );
            }
        });
    ...
    _flattenSensorConfiguration(configuration, sensorType) {
        ...
        if (numberOfSensorDataTypes > 0) {
            const flattenedConfiguration = new DataView(new ArrayBuffer(numberOfSensorDataTypes * 3));
            let byteOffset = 0;
            for (const sensorDataType in _configuration) {
                flattenedConfiguration.setUint8(byteOffset, sensorDataTypes[sensorDataType]);
                flattenedConfiguration.setUint16(byteOffset + 1, _configuration[sensorDataType], true);
                byteOffset += 3;
            }
    ...
    _parseSensorData(dataView, byteOffset = 0) {
        ...
        while (byteOffset < dataView.byteLength) {
            ...
            byteOffset = this._parseSensorDataType(dataView, byteOffset, timestamp, sensorType);
        }
    ...
    _parseSensorDataType(dataView, byteOffset, timestamp, sensorType) {
        ...
        switch (sensorType) {
            case this.SensorTypes.MOTION:
                byteOffset = this._parseMotionSensorData(dataView, byteOffset, finalByteOffset, timestamp);
                break;
            case this.SensorTypes.PRESSURE:
                byteOffset = this._parsePressureSensorData(dataView, byteOffset, finalByteOffset, timestamp);
                break;
        }
    ...
    _parseMotionSensorData(dataView, byteOffset, finalByteOffset, timestamp) {
        while (byteOffset < finalByteOffset) {
            ...
            const motionSensorDataTypeString = this.MotionDataTypeStrings[motionSensorDataType];
            ...
            switch (motionSensorDataType) {
                ...
                case this.MotionDataTypes.quaternion:
                    quaternion = this._parseMotionQuaternion(dataView, byteOffset, scalar);
            ...
            this.dispatchEvent({
                ...
                message: {
                    ...
                    [motionSensorDataTypeString]:
                        motionSensorDataTypeString == "quaternion" ? quaternion : vector || euler,
                    rawData,
                },
            });
            byteOffset += byteSize;
    ...
    _parsePressureSensorData(dataView, byteOffset, finalByteOffset, timestamp) {
        while (byteOffset < finalByteOffset) {
            ...
            const pressureSensorDataTypeString = this.PressureDataTypeStrings[pressureSensorDataType];
            this.log(`got pressure sensor data type "${pressureSensorDataTypeString}"`);
            ...
            switch (pressureSensorDataType) {
                ...
                case this.PressureDataTypes.pressureDoubleByte:
                    ...
                    for (let index = 0; index < 16; index++) {
                        let value;
                        if (pressureSensorDataType == this.PressureDataTypes.pressureSingleByte) {
                        ...
                        const { x, y } = this.getPressurePosition(index, this.isRightInsole);
    ...
    get MotionDataScalars() {
        return this._isUsingBNO08x ? this.constructor._MotionDataScalars : this.constructor.MotionDataScalars;
    }
            ...
            case "object":
                switch (datum.constructor.name) {
                    ...
                    case "DataView":
                        ...
                        break;
                    case "Object":
                        this.log("uncaught datum type: object (what do we do with the keys and in what order?)", datum);
                        break;
                }
                break;
            case "string":
                return this._concatenateArrayBuffers(Uint8Array.from([datum.length]), this.textEncoder.encode(datum));
                break;
            case "number":
            case "boolean":
    ...
    async vibrateSequence(sequence) {
        ...
        return this._vibrate([
            ...
            ...sequence.map((value, index) => (index % 2 ? Math.floor(value / 10) : value)),
        ]);
    }
}
Object.assign(BaseMission, {
    ...
    MotionCalibrationTypeStrings: ["system", "gyroscope", "accelerometer", "magnetometer"],
    _MotionCalibrationTypeStrings: ["accelerometer", "gyroscope", "magnetometer", "quaternion"],
    ...
    PressureDataTypeStrings: ["pressureSingleByte", "pressureDoubleByte", "centerOfMass", "mass", "heelToToe"],
...
["Type", "MotionCalibrationType", "MotionCalibrationValue", "MotionDataType", "PressureDataType", "SensorType"].forEach(
    (name) => {
        BaseMission[name + "s"] = BaseMission[name + "Strings"].reduce((object, name, index) => {
            object[name] = index;
            return object;
        }, {});
    }
...

{
    ...
    BaseMission.InsoleCorrectionQuaternions.right.setFromEuler(insoleCorrectionEuler);

    insoleCorrectionEuler.set(-Math.PI / 2, Math.PI / 2, 0);
    BaseMission.InsoleCorrectionQuaternions.left.setFromEuler(insoleCorrectionEuler);
}
{
    const bno085CorrectionEuler = new THREE.Euler();
    bno085CorrectionEuler.set(0, -Math.PI / 2, 0);
    BaseMission.bno085CorrectionQuaternions.motionModule.setFromEuler(bno085CorrectionEuler);

    bno085CorrectionEuler.set(0, Math.PI, 0);
    BaseMission.bno085CorrectionQuaternions.insoles.left.setFromEuler(bno085CorrectionEuler);
    bno085CorrectionEuler.set(0, Math.PI, 0);
    BaseMission.bno085CorrectionQuaternions.insoles.right.setFromEuler(bno085CorrectionEuler);
}
...
BaseMission.permuteQuaternion = function (quaternion) {
    const permutation = this.quaternionPermutations[this.quaternionPermutationIndex];
